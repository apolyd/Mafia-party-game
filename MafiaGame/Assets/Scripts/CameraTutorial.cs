...
public class CameraTutorial : Photon.MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit hit;
        string LookAtTutorial;
        if (Physics.Raycast(ray, out hit, 10f))
        {
            var selection = hit.transform;
            Debug.Log("Vlepw to " + selection.gameObject.tag);
            LookAtTutorial = selection.gameObject.tag;
            if (LookAtTutorial != "Untagged")//if u are looking at something that is tagged display it at the screen
            {
                CanvasTutorial.transform.Find("GlobalText1Tutorial").gameObject.GetComponent<Text>().text = "You are looking at " + LookAtTutorial;
            }
            else
            {
                CanvasTutorial.transform.Find("GlobalText1Tutorial").gameObject.GetComponent<Text>().text = " ";
            }
        }
